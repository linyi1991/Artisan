...
namespace Artisan.CraftingLists
{
    internal class CraftingListUI
    {
        ...
        {
            ImGui.TextWrapped("製作清單可以把多種物品排入佇列並逐一製作。可以用下方按鈕從 Teamcraft 匯入，或點擊「+」建立並命名新清單。也可在遊戲製作筆記中對物品按右鍵，將它加入現有清單或建立新清單。");

            ImGui.Dummy(new Vector2(0, 14f));
            ImGui.TextWrapped("左鍵點擊清單可開啟編輯器；右鍵點擊可只選取清單而不開啟編輯器。");
        ...
        private static void DrawListOptions()
        {
            ...
            if (selectedList.ID != 0)
            {
                ...
                if (RetainerInfo.ATools)
                {
                    ...
                    {
                        if (ImGui.Button("停止從僱員領取材料", new Vector2(ImGui.GetContentRegionAvail().X, 30)))
                        {
                    ...
                    else
                    {
                        ...
                        {
                            if (ImGui.Button("從僱員補充材料", new Vector2(ImGui.GetContentRegionAvail().X, 30)))
                            {
                ...
                else
                {
                    if (!RetainerInfo.AToolsInstalled)
                        ImGuiEx.TextCentered(ImGuiColors.DalamudYellow, "請安裝 Allagan Tools 以使用僱員功能。");

                    if (RetainerInfo.AToolsInstalled && !RetainerInfo.AToolsEnabled)
                        ImGuiEx.TextCentered(ImGuiColors.DalamudYellow, "請啟用 Allagan Tools 以使用僱員功能。");
                }
            ...
            if (ImGui.Button("從剪貼簿匯入清單（Artisan 匯出格式）", new Vector2(ImGui.GetContentRegionAvail().X, 30)))
            {
                try
                {
                    ...
                    if (clipboard != string.Empty)
                    {
                        ...
                        {
                            Notify.Error("剪貼簿內容不是有效的 Artisan 清單。");
                        }
                    }
                    else
                    {
